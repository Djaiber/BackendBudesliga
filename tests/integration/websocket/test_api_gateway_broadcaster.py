...
@pytest.mark.asyncio
async def test_send_to_connection_serializes_message(broadcaster: ApiGatewayBroadcaster, mock_apigw_client):
...
@pytest.mark.asyncio
async def test_send_to_connection_with_empty_message(broadcaster: ApiGatewayBroadcaster, mock_apigw_client):
...
async def test_broadcast_sends_messages_in_parallel(
    ...
    connections = [
        {"conn_id": f"conn-{i}", "user_id": f"user-{i}", "room_id": "room-123"}
        for i in range(10)
    ]
