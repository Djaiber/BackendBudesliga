...
async def test_submit_prediction_success(
    ...
    assert predictions[0].user_id == "p1"
...
async def test_submit_to_closed_window(
    ...
    window = PredictionWindow(
        ...
        game="NEXT_GOAL_TIMING",
        prompt="When will the next goal be scored?",
        opened_at_ms=clock.now_ms() - 30000,
        deadline_ms=clock.now_ms() - 10000,
        options=("0-15 min", "15-30 min", "30-45 min"),
        status="closed",
    )
    await window_repo.save(window)
...
async def test_submit_to_expired_window(
    ...
    window = PredictionWindow(
        ...
        prompt="When will the next goal be scored?",
        opened_at_ms=clock.now_ms() - 30000,
        deadline_ms=clock.now_ms() - 1000,  # Expired
        options=("0-15 min", "15-30 min", "30-45 min"),
        status="open",
    )
    await window_repo.save(window)
...
async def test_multiple_players_can_submit(
    ...
    window = PredictionWindow(
        ...
        opened_at_ms=clock.now_ms(),
        deadline_ms=clock.now_ms() + 20000,
        options=None,  # No options for numeric prediction
        status="open",
    )
    await window_repo.save(window)
